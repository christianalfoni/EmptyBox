...
Mobx truly is magical in the way our components detect a need for render by just replacing values on our class instances. It is easy and Mobx has a low learning curve.
...
How much code do we have to write? It is important to understand that less code does not mean better code. We could state that type checking is boilerplate, but it gives us guarantees and arguably more readability of our code. Abstractions have the same nature. We write less code, but often abstractions can hide what is really going on and make the code less readable for the next developer.
...
In this component we are not defining what state the component depends on, it automagically understands that by accessing observable properties. It is easy to read the code, but it is harder to figure out what causes this component to actually render.
...
There has been a revolution of developer tools in the React ecosystem. One thing is the React debugger itself, but especially when Redux got all its attention with immutabilty it opened up new possibilities. Especially the time travel debugger got a lot of attention. Time travel was actually one of the early experiments of the Cerebral debugger, almost 3 years ago, but it has ended up as a gimmick. The time travel itself is not the most valuable part; it is the history of state changes, and ideally how these changes came to be, that is really helpful. The mental image is where we find most value.

**MOBX**

Mobx has developer tool that "does the job", as Preethi says. We can investigate renders and what state properties a specific component depends on. This is done in the browser as an overlay.
...
The Cerebral debugger is taken even further. Even though Redux lists mutations, we do not know how they relate to each other and how they came to be. In Cerebral we not only get an overview of mutations, but also an overview of the complete flow of changes in our application. The debugger itself is a standalone application that allows us to connect to any JS environment, whether browser, server, React Native, Electron etc. We can even combine client and server side execution in a single operation flow.
...
Redux, with its explicit definition of what a state change can be and how it is made, is the "king of predictability". Based on the ideas of Flux, Redux has become the THE implementation of Flux and is therefore very predictable.
...
Redux does not really have a concept of modularity. We define our actions somewhere over here and our reducers somewhere over there. The great thing about this, is that any action can trigger any state changes in any reducer, and any reducer can react to any action. There are some issues though, because the reducers themselves are modular, meaning that a reducer can not access the state of another reducer. There are of course ways to handle these scenarios.

**CEREBRAL**

Cerebral has a concept called modules. This is a way for us to structure our application without isolating signals and state: any signal can change and grab any state. Also any signal can compose its logic from other signals. That means Cerebral is highly composable and modular, without the isolation. There is no way to go wrong planning out the domains and no risk having circular dependency issues nor any need to pass class instances around to get access to what we need.
...
Mobx is very easy to get going with, but it does not force us into a strict pattern of where to render, where to request state changes and where to make those state changes. It can happen anywhere. That makes Mobx, without good discipline, less than ideal for scalability and maintenance.
